...
- **DocumentDB 主庫 + Atlas 備援**：主庫不可用時啟動自動 fallback 到 Atlas。可由 super_admin 在前端觸發 DocumentDB → Atlas 同步（JTI/HCIoT/ESG 同步自身、General 全域），或災後反向把 Atlas 資料補回主庫（衝突以主庫為準）。詳見 `scripts/README_db_sync.md`。
...
FastAPI backend
  ...
  - ESG runtime, quiz, prompt, knowledge APIs
  - General chat, per-store topics/images/knowledge, prompt, API key APIs
...
│   ├── routers/
│   │   ├── general/                    # 通用 chat、per-store topics/images/knowledge、prompt、API key
...
│   │   ├── general/                    # General managed-app runtime、per-store topic/image/knowledge stores
│   │   ├── jti/                        # JTI agent、quiz、knowledge、TTS glue
│   │   ├── hciot/                      # HCIoT agent、topic/image/knowledge stores、TTS glue
│   │   ├── esg/                        # ESG agent、quiz、knowledge、TTS glue
...
- `POST /api/esg/chat/start`, `POST /api/esg/chat/message`
- `POST /api/jti/quiz/start`, `POST /api/jti/quiz/pause`
- `POST /api/esg/quiz/start`, `POST /api/esg/quiz/pause`
- `GET /api/hciot/topics/{lang}`, `GET /api/hciot/topics/{lang}/all`
- `GET /api/general/stores/{store_name}/topics/{lang}`
- `GET /api/hciot/images/{image_id}`, `GET /api/general/stores/{store_name}/images/{image_id}`
- `POST /v1/chat/completions`

Admin:

- `GET /api/jti-admin/conversations`, `GET /api/hciot-admin/conversations`, `GET /api/esg-admin/conversations`（讀取/匯出開放給該應用的 user；刪除限 admin）
- `GET/POST/PUT/DELETE /api/jti-admin/knowledge/...`
- `GET/POST/PUT/DELETE /api/hciot-admin/knowledge/...`
- `GET/POST/PUT/DELETE /api/esg-admin/knowledge/...`
...
- `GET/POST/PUT/DELETE /api/esg-admin/quiz-bank/...`
- `GET/POST/PUT/DELETE /api/general/stores/{store_name}/knowledge/...`
