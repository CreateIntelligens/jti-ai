...
import type {
  ...
  QuizBankStats,
  QuizBankApi,
} from '../../services/api/_shared/quizBank';

interface QuizManagementTabProps {
  language: string;
  api: QuizBankApi;
  appContext?: 'jti' | 'general';
}

type SubTab = 'questions' | 'results';

const EMPTY_OPTION: QuizQuestionOption = { id: '', text: '', score: {} };

export default function QuizManagementTab({ language, api, appContext: _appContext = 'jti' }: QuizManagementTabProps) {
       ...
       const [stats, setStats] = useState<QuizBankStats | null>(null);
       ...
       const [editQuizData, setEditQuizData] = useState<Partial<QuizResult>>({});
       ...
       const loadQuestions = useCallback(async () => {
              ...
              try {
                     ...
                     const sortedQuestions = (qData.questions || []).sort((a: QuizQuestion, b: QuizQuestion) =>
       ...
       }, [language, api, selectedQuizSetId]);
       ...
       }, [language, selectedQuizSetId, api]);
       ...
       // --- Quiz Result CRUD ---
       const handleStartQuizEdit = (qr: QuizResult) => {
