...
import { NEW_VALUE, buildCategoryOptions, buildTopicOptions, categoryPrefix, createEmptyDraft, draftFromFile, getErrorMessage, getMetadataPayload, normalizeLabel, slugify, type FileMetadataDraft, type TopicLabels } from './topicUtils';
import { useEscapeKey } from '../../../hooks/useEscapeKey';
import { buildExplorerTree, filterExplorerNodes, flattenExplorerNodes, getCurrentPathLabel, readExpandedKeys, writeExpandedKeys } from './explorer/explorerTree';
...
export default function QaKnowledgeWorkspace({
  ...
  const pollReindexStatus = (sourceType: 'hciot' | 'jti' | string) => {
    clearReindexTimer();
    let attempts = 0;
    ...
    reindexTimerRef.current = window.setInterval(async () => {
      attempts += 1;
      try {
        const res = await api.getReindexStatus(sourceType);
  ...
  const handleReindex = async () => {
    ...
    setReindexing(true);
    try {
      await api.reindex(config.sourceType);
      showStatus(text('重新索引已開始', 'Reindexing started'));
      pollReindexStatus(config.sourceType);
    } catch (error) {
  ...
  const resolveDraftBeforeSave = async (currentDraft: FileMetadataDraft): Promise<FileMetadataDraft> => {
    ...
    const topicData = await api.listTopicsAdmin(language);
    ...
    >
      ...
      {selectedImageName ? (
        ...
        <FileDetailPane
          ...
          actions={{
            onDownload: () => {
              if (selectedFile) api.downloadKnowledgeFile(selectedFile.name, language);
            },
