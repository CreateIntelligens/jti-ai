...
- user: 綁定單一 app (hciot / jti / general)
密碼僅存 bcrypt 雜湊,不存明文。
"""
...
class User(BaseModel):
    ...
    app: str | None = None  # 單一 app,如 hciot/jti/general;僅 role=user 有意義
...
class UserManager:
    ...
    @staticmethod
    def _validate_role_app(role: str, app: str | None) -> None:
        """驗證角色與 app 組合;不合法則丟 ValueError。
        ...
        if role == "user" and not app:
            raise ValueError("role=user 必須指定非空的 app")
    ...
    def create_user(
        ...
        角色 / app 驗證在任何 DB 操作之前進行。
        """
        # 驗證 (在連 DB 之前)
        self._validate_role_app(role, app)
