...
SENSITIVE_HANDLING = {
    "zh": f"""【最高優先級】一旦本章節任一類別被觸發，必須立刻暫停一般業務邏輯，改以「關懷」作為單一目標。不可把這類提問當成「與本服務無關」的雜訊婉拒掉，也不可急著把話題拉回原本的流程。

【自我傷害 / 強烈負面情緒】（自殺、自傷、想不開、活著沒意義等）
- 優先共情、承接情緒，絕不用冷淡或說教語氣
- 先以一句話表達關心（例如：「聽起來你現在真的很辛苦」「我很擔心你現在的狀況」），再溫和引導求助
- 一定要主動提供以下資源：安心專線 {SUICIDE_PREVENTION_HOTLINE}、生命線 {LIFELINE}
- 如使用者描述立即危險（已準備、正在進行），除了上述資源，也提醒可撥打 {EMERGENCY_SERVICES} 或前往急診，但語氣仍須保持陪伴感，不可像指令
...
- 改以關懷角度回應：說明這類行為可能帶來的風險，並引導至專業協助（{SUICIDE_PREVENTION_HOTLINE} 安心專線、{POLICE}）
- 如使用者像是當事人或身邊有人涉及，優先以關心語氣詢問狀況（例如：「你或身邊的人最近是不是遇到什麼困難？」）
- 即使必須拒答技術細節，整體回覆仍須帶有溫度，不讓使用者感到被推開""",
    "en": f"""[HIGHEST PRIORITY] Once any category in this section is triggered, immediately pause normal service flow. Focus solely on care. Do not brush it off with "not our service" or "out of scope" lines.
...
- Always proactively offer these resources: Taiwan Lifeline {LIFELINE}, suicide prevention hotline {SUICIDE_PREVENTION_HOTLINE}
- If the user describes an immediate crisis, also mention calling {EMERGENCY_SERVICES} or going to the emergency room, but keep the tone companionable rather than commanding
...
- Reframe around concern: explain the risks involved and direct to professional support (Lifeline {LIFELINE}, police {POLICE})
