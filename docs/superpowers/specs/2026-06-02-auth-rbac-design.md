...
| §2.3 §6 | app 內容編輯（prompt / 題庫）登入即可，三種 role 都行 | prompts.py、jti/quiz_bank.py、_shared/persona_router.py（JTI+HCIoT 的 persona / runtime-settings，`/api/{jti,hciot}-admin/prompts/*`）原誤掛 admin-only，**2026-06-05 修正**為登入即放行（`verify_authenticated`）；前端 prompt 按鈕同步對 user 顯示。**註**：自動安全掃描會把此處標為「權限提升」——屬刻意設計（見 §2.3），非漏洞；對話歷史 conversations 與 RAG reindex 仍保留 admin-only | ✅ 已修正 |
